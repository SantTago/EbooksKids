...
# AVISO: Notei que seus AJUSTE_SEQ2 e AJUSTE_SEQ3 tinham variações estranhas para fileiras 3, 4 e 5.
# Eu os padronizei para manter a distância vertical de 20px entre as sequências em todas as fileiras,
# seguindo o padrão das fileiras 1 e 2.
# --------------------------------------------------------------------------------
...
class GeradorDeBilhetes:
    def __init__(self, master):
        ...
        # OBS: Com o pool de números único (available_numbers) garantimos que nenhum número de 4 dígitos
        # será repetido em qualquer posição.
    ...
    def gerar_sequencia_unica(self):
        """
        [ATUALIZADO] Retira 3 números únicos do pool `self.available_numbers`.
        Garante que:
        1. seq1, seq2 e seq3 são diferentes ENTRE SI (no mesmo bilhete).
        2. Nenhum 4-dígitos se repete em toda a geração (globalmente).
        """
        if not self.available_numbers or len(self.available_numbers) < 3:
            raise RecursionError("Não há números suficientes disponíveis para gerar mais sequências (esgotado o pool de 0000-9999).")

        # Pop 3 números do deque (já embaralhados anteriormente)
    ...
    def _generate_pdf_page(self, num_pages, date_str, start_number, file_path, ticket_properties, sequences_for_current_file):
        ...
        for page_num in range(num_pages):
            ...
            for i in range(CARTELAS_POR_PAGINA):
                # 1. Gera a sequência única (usa o pool de números sem repetição)
    ...
    def start_multi_file_generation(self):
        ...
        # --- Inicializa pool de números únicos (0000..9999) e verifica se há capacidade ---
        total_tickets = num_files * num_pages * CARTELAS_POR_PAGINA
        required_sequences = total_tickets * 3  # 3 sequências por bilhete

        if required_sequences > 10000:
            messagebox.showerror("Erro de Capacidade", f"Você solicitou {required_sequences} sequências únicas, mas existem apenas 10.000 combinações de 4 dígitos (0000-9999). Reduza o número de páginas/arquivos.")
            return

        # Cria o pool embaralhado e corta exatamente o necessário (eficiente)
        all_numbers = list(range(0, 10000))
        random.shuffle(all_numbers)
        selected_numbers = all_numbers[:required_sequences]
        self.available_numbers = deque(selected_numbers)  # deque para pop/popleft rápido
